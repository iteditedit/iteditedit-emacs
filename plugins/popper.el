...
(defun pop-to-buffer (buffer &optional other-window on-frame)
...
window even if BUFFER is already visible in the selected window."
  (let ((parent (popper-parent)))
	...
	(progn
	  (popper-bury-output)
	  (funcall popper-pop-to-buffer buffer other-window on-frame)
	  (select-window parent)
	  (popper-show-output)
	  (sit-for 0)			;Allow display update
	  (funcall popper-pop-to-buffer buffer nil on-frame))
	(funcall popper-pop-to-buffer buffer other-window on-frame))))
